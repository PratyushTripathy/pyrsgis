...
def trim_array(data_arr, remove='negative'):
    """
    Remove unwanted value from the edge
    ...
    ----------
    data_arr         : array
                       A 2D array to clip. Ideally this should be a raster array that has
    ...
    In the above example a raster file which was masked using a polygon but the 'match extent
    of the raster with extent of the input polygon' option was disabled has been used. In this
    particular case, useful values were only towards a corner of the rastersurround by NoData cells.
    Hence, the actual extent of the file was much larger (and unnecessary).
...
def trim(ds, array, value):
    # identify clipping coordinates
    # call clip function
    return None

def trim_file(filename, value, outfile):
    # read the file
    # call trim function
    # export the outfile
    return None
